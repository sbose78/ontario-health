...
**Setup Requirements:**
1. Private key in `~/.snowflake/ontario_health_key.p8` (already generated by `make setup`)
2. Service account created in Snowflake (run `sql/migrations/005_create_service_account.sql`)
3. Python 3.11+
4. Network policy allows your IP

**⚠️ Important**: You must run `sql/migrations/005_create_service_account.sql` in Snowflake before ingestion works.
